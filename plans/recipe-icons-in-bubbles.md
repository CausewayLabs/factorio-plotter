...
- **Model:** Sonnet
- **Description:** Adds two outputs to `scripts/generate-bundled-recipes.mjs`: (1) `src/recipes/icons.webp` — FactorioLab's pre-built sprite sheet downloaded as a binary file from `https://raw.githubusercontent.com/factoriolab/factoriolab/main/src/data/spa/icons.webp`, and (2) `src/recipes/iconAtlas.json` — a `Record<iconId, { x: number; y: number }>` map. **Note: FactorioLab's `data.json` does NOT contain per-item `x/y/w/h` coordinates.** Instead it has a top-level `icons` array of the form `[{ id, position: "-66px 0px", color }]` where `position` is a CSS `background-position` string for a fixed 66×66 sprite grid. The generator must parse each `position` string (`"-66px 0px"` → `{ x: 66, y: 0 }`, negating the negative pixel offsets) and emit `{ x, y }` per icon id. Sprite size is constant 66 — encode it as a single exported `SPRITE_SIZE = 66` constant in the generator and write it into the atlas file's wrapper (e.g. `{ spriteSize: 66, entries: { ... } }`) so consumers don't hardcode it. Icon-id resolution for a recipe: prefer `recipe.icon` if present, else fall back to the recipe's first output item id; the atlas key space is item/icon ids, not recipe ids.
- **Scope:** Only `scripts/generate-bundled-recipes.mjs`. Does not touch `bundled.json` schema, source types, or any React component.
- **Acceptance Criteria:**
  ...
  - `iconAtlas.json` has shape `{ spriteSize: 66, entries: Record<string, { x: number; y: number }> }` and contains entries for common items (e.g. `"iron-plate"`, `"copper-plate"`, `"electronic-circuit"`).
  - Position parsing correctly handles the `"-Npx -Mpx"` format and produces non-negative integers.
...
- **Model:** Haiku
- **Description:** Creates a new module `src/recipes/iconAtlas.ts` that statically imports `iconAtlas.json`, exports a typed `IconAtlasEntry` (`{ x: number; y: number }`) and `SPRITE_SIZE` (re-exported from the JSON's `spriteSize` field), and exports a pure `getIconCoords(iconId: string): IconAtlasEntry | undefined` lookup function. Also exports the bundled sprite-sheet URL via Vite's `?url` asset import (e.g. `import iconsUrl from './icons.webp?url'`) so consumers reference a single canonical source. Adds a `resolveRecipeIconId(recipe: Recipe): string` helper that returns `recipe.icon ?? recipe.outputs[0]?.itemId` so the bubble component doesn't duplicate that fallback logic. Does NOT add anything to the Zustand `useRecipeStore` — the atlas is static, stateless data and does not belong in the persisted store.
...
- **Model:** Sonnet
- **Description:** In `BubbleLayer.tsx`, replaces the `<text>` label element inside `BubbleNode` with an SVG `<image>` that renders the primary product's sprite from the bundled sheet. Uses a `<clipPath>` circle (radius = `BUBBLE_RADIUS - 4` for a 4 px padding ring) to clip the image to the bubble shape. Imports `iconsUrl`, `SPRITE_SIZE`, `getIconCoords`, and `resolveRecipeIconId` from `src/recipes/iconAtlas.ts`. The full sprite sheet is rendered as a large `<image href={iconsUrl}/>` translated so the chosen sprite cell lands centered on the bubble: `x = bubble.cx - coords.x - SPRITE_SIZE/2`, `y = bubble.cy - coords.y - SPRITE_SIZE/2` (image renders at natural size; only the clipped region is visible). Adds a `<title>` child as the FIRST child of the wrapping `<g data-bubble-id>` element containing `label` so the browser renders a native tooltip on hover. If `getIconCoords(resolveRecipeIconId(recipe))` returns `undefined`, falls back to the existing truncated `<text>` label (no silent blank bubble).
